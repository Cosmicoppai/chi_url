...
async def add_user(user: User, background_tasks: BackgroundTasks):
    ...
    if not exist:
        ...
        session.execute(user_add_stmt,
                        [user.username, user.email, _password])  # replace the pre_stmt with the actual values
...
async def check_user(username: str = Path(..., title="username", description="username of the user"),
                     email: str = None):  # Check if user exists or not.
    try:
        if email:
